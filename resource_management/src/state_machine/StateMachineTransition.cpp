...
void StateMachineTransition::analyse(const std::string& state_name)
{
  if(time_out_ == ros::Duration(0))
    std::cout << "\t\t" << ros::this_node::getName() << "[WARNING] state " << state_name << ": transition timeout set to 0" << std::endl;

  if(duration_ == ros::Duration(0))
    std::cout << "\t\t" << ros::this_node::getName() << "[WARNING] state " << state_name << ": transition duration set to 0" << std::endl;
}
